...
class SQLTransport extends TransportStream {
  ...
  cleanup(override = false) {
    ...
    try {
      await this.client(this.tableName).where("timestamp", "<", moment().utc().subtract(this.daysToKeep, "day").format(DATE_FORMAT)).del();
  ...
  async clear() {
    try {
      ...
      this.log({ message: "Log clear failed", timestamp: moment().utc().format(DATE_FORMAT), level: "error", source: "logs" }, () => {});
    }
  }
